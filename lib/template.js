...
Template.prototype.eval = function(src, model, options, ctx) {
  ...
  var fn = this.exec(src, options, ctx);
...
Template.prototype.src = function(src, compileOptions) {
  return [
    '[function(c) {',
    'c.m = this;',
    'var sp = c.stack.length, content = c._content, extend = c._extend, partial = c._partial, defaultContent = c._defaultContent, append = c._append, prepend = c._prepend;',
...
Template.prototype.compile = function(src, compileOptions) {
  ...
  var fn = this.exec(src, compileOptions);
