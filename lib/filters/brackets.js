...
var _blockRe = /^(case|default)\b/;
var _wrapCondRe = /^(for|switch|catch|while|if|else\s+if)\s+(?!\()((\S|\s\S)*)\s*$/;
var _ifRe = /^(if|switch|while|for|else|finally|catch)\b/;
var _callbackRe = /(function\s*\([^\)]*\)\s*)[^\{]/;
...
BracketsProto._expandCallback = function(code, content) {
  ...
  if (!_blockRe.test(code) && !this._isEmptyExp(content)) {
    if (!_ifRe.test(code)) {
      m = _callbackRe.exec(code);
      if (m) {
