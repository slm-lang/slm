...
var _parseCommentBlock = function(parser) {
  while (parser._lines.length) {
    var indent = parser._emptyLineRe.test(parser._lines[0]) ? 0 : parser._getIndent(parser._lines[0]);
...
ParserProto._parseLineIndicators = function() {
  for (;;) {
    ...
    if (
      // HTML comment
      this._matchLineThen(this._htmlCommentRe, _parseHtmlComment) ||
      // or HTML conditional comment
      this._matchLineThen(this._htmlConditionalCommentRe, _parseHtmlConditionalComment) ||
      // Slm comment
      this._ifTrueThen(firstChar === '/', _parseCommentBlock) ||
      // Text block.
      this._matchLineThen(this._textBlockRe, _parseTextBlock) ||
      // Inline html
      this._ifTrueThen(firstChar === '<', _parseInlineHtml) ||
      // Code block.
      this._ifTrueThen(firstChar === '-', _parseCodeBlock) ||
      // Output block.
      this._matchLineThen(this._outputBlockRe, _parseOutputBlock) ||
      // Embedded template.
      this._matchLineThen(this._embededRe, _parseEmbeded) ||
      // Doctype declaration
      this._matchLineThen(this._doctypeRe, _parseDoctype) ||
      // HTML tag
