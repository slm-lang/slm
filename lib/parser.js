...
function Parser() {
  this._file = this._line = this._origLine = null;
  this._lineno = 0;
  this._lines = [];
  this._indents = [0];
  ...
  this._quotedAttrRe = new RegExp(attrName + '\\s*=(=?)\\s*("|\')');
  this._codeAttrRe = new RegExp(attrName + '\\s*=(=?)\\s*');
}
...
var _parseCommentBlock = function(parser) {
  while (parser._lines.length) {
    var indent = emptyLineRe.test(parser._lines[0]) ? 0 : parser._getIndent(parser._lines[0]);
...
ParserProto._parseJSCode = function(outerDelimeter) {
  ...
  while (this._line.length && (count || !endRe.test(this._line))) {
    if (nextLineRe.test(this._line)) {
      ...
      } else if (m = delimRe.exec(this._line)) {
