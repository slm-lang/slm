...
Parser.prototype.escapeRegExp = function(str) {
  return str.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&");
}

Parser.prototype.reset = function(lines, stacks) {
  this.stacks = stacks || [];
  this.indents = [0];
...
Parser.prototype.pushOnTop = function(item) {
...
Parser.prototype.getIndent = function(line) {
  var m = line.match(/^[ \t]+/);
  return m ? m[0].replace(/\t/g, ' ').length : 0
};

Parser.prototype.exec = function(str) {
  var result = ['multi'];
  this.reset(str.split(/\r?\n/), [result]);
...
Parser.prototype.parseLineIndicators = function() {
  do {
...
Parser.prototype.parseAttributes = function(attributes) {
...
Parser.prototype.parseTextBlock = function(firstLine, textIndent, inTag) {
  var result = ['multi'];
...
Parser.prototype.parseCommentBlock = function() {
  while (this.lines.length) {
    var indent = /^\s*$/.test(this.lines[0]) ? 0 : this.getIndent(this.lines[0]);
...
Parser.prototype.parseBrokenLine = function() {
...
Parser.prototype.parseJSCode = function(outerDelimeter) {
...
Parser.prototype.parseQuotedAttribute = function(quote) {
  var value = '', count = 0;
...
Parser.prototype.syntaxError = function(message) {
  throw new Error(message + ' line: ' + this.lineno);
};

Parser.prototype.expectNextLine = function() {
  if (this.nextLine()) {
