...
var _attrDelimRe = /^\s*([\(\)\[\]])/;
var _blockExpressionRe = /^\s*:\s*/;
var _closedTagRe = /^\s*\/\s*/;
var _delimRe = /^[\(\)\[\]]/;
var _doctypeRe = /^doctype\b/i;
var _embededRe = /^(\w+):\s*$/;
var _emptyLineRe = /^\s*$/;
var _htmlCommentRe = /^\/!(\s?)/;
var _htmlConditionalCommentRe = /^\/\[\s*(.*?)\s*\]\s*$/;
var _indentRegex  = /^[ \t]+/;
var _indentationRe = /^\s+/;
var _newLineRe = /\r?\n/;
var _nextLineRe = /[,\\]$/;
var _outputBlockRe = /^=(=?)([<>]*)/;
var _outputCodeRe  = /^\s*=(=?)([<>]*)/;
var _tabRe = /\t/g;
var _textBlockRe = /^((\.)(\s|$))|((\|)(\s?))/;
var _textContentRe = /^( ?)(.*)$/;
...
function Parser() {
  ...
  this._quotedAttrRe = new RegExp(_attrName + '\\s*=(=?)\\s*("|\')');
  this._codeAttrRe = new RegExp(_attrName + '\\s*=(=?)\\s*');
}
...
ParserProto._getIndent = function(line) {
  var m = line.match(_indentRegex);
  return m ? m[0].replace(_tabRe, ' ').length : 0;
};
...
ParserProto.exec = function(str, options) {
  ...
  var res = ['multi'];
  this._reset(str.split(_newLineRe), [res]);
...
ParserProto._parseLine = function() {
  if (_emptyLineRe.test(this._line)) {
  ...
  this._line = this._line.replace(_indentationRe, '');
...
var _parseCommentBlock = function(parser) {
  while (parser._lines.length) {
    var indent = _emptyLineRe.test(parser._lines[0]) ? 0 : parser._getIndent(parser._lines[0]);
...
ParserProto._matchLineThen = function(regex, next) {
...
ParserProto._ifTrueThen = function(condition, next) {
  if (condition) {
    next(this);
    return true;
  }
...
ParserProto._parseLineIndicators = function() {
  for (;;) {
    ...
    if (
      // HTML comment
      this._matchLineThen(_htmlCommentRe, _parseHtmlComment) ||
      // or HTML conditional comment
      this._matchLineThen(_htmlConditionalCommentRe, _parseHtmlConditionalComment) ||
      // Slm comment
      this._ifTrueThen(firstChar === '/', _parseCommentBlock) ||
      // Text block.
      this._matchLineThen(_textBlockRe, _parseTextBlock) ||
      // Inline html
      this._ifTrueThen(firstChar === '<', _parseInlineHtml) ||
      // Code block.
      this._ifTrueThen(firstChar === '-', _parseCodeBlock) ||
      // Output block.
      this._matchLineThen(_outputBlockRe, _parseOutputBlock) ||
      // Embedded template.
      this._matchLineThen(_embededRe, _parseEmbeded) ||
      // Doctype declaration
      this._matchLineThen(_doctypeRe, _parseDoctype) ||
      // HTML tag
      this._matchLineThen(this._tagRe, _parseTag)) {
        this._pushOnTop(['newline']);
        return;
      }
...
ParserProto._parseTag = function(tag) {
  ...
  do {
    // Block expansion
    m = _blockExpressionRe.exec(this._line);
    if (m) {
      this._sliceLine(m[0].length);
    ...
    // Handle output code
    m = _outputCodeRe.exec(this._line);
    if (m) {

      this._sliceLine(m[0].length);
      var trailingWS2 = m[2].indexOf('>') !== -1;
      var leadingWS2 = m[2].indexOf('<') !== -1;
    ...
    m = _closedTagRe.exec(this._line);
    if (m) {
      this._sliceLine(m[0].length);
    ...
    // Text content
    m = _textContentRe.exec(this._line);
    if (m) {
...
ParserProto._parseAttributes = function(attributes) {
  ...
  m = _attrDelimRe.exec(this._line);
  if (m) {
  ...
  if (delimiter) {
    booleanAttrRe = new RegExp(_attrName + '(?=(\\s|' + this._escapeRegExp(delimiter) + '|$))');
...
ParserProto._parseTextBlock = function(firstLine, textIndent, inTag) {
  ...
  while (this._lines.length) {
    if (_emptyLineRe.test(this._lines[0])) {
      this._nextLine();
      result.push(['newline']);
...
ParserProto._parseBrokenLine = function() {
  ...
  while (m = _nextLineRe.exec(brokenLine)) {
...
ParserProto._parseJSCode = function(outerDelimeter) {
  ...
  while (this._line.length && (count || !endRe.test(this._line))) {
    if (_nextLineRe.test(this._line)) {
      code += this._line + '\n';
      this._expectNextLine();
    } else {
      if (count > 0) {
      ...
      } else if (m = _delimRe.exec(this._line)) {
