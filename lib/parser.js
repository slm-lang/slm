...
function Parser(attrDelims) {
  ...
  this._hashAttrName = '^\\s*(#(?!\\${)[^\\0\\"\'><\\/=\\s\\.#' + attrDelimsStr+ ']+)';
  this._quotedAttrRe = new RegExp(this._attrName + '\\s*=(=?)\\s*("|\')');
  this._hashAttrRe = new RegExp(this._hashAttrName + '(?:\\s*=(=?)\\s*("|\'))?');
...
p._parseAttributes = function(attributes) {
  ...
  while (true) {
    ...
    m = this._hashAttrRe.exec(this._line);
    if (m) {
      var attrValue;
      this._sliceLine(m[0].length);
      if (m[3]) {
        attrValue = ['escape', !m[2].length, ['slm', 'interpolate', this._parseQuotedAttribute(m[3])]];
      } else {
        attrValue = ['multi'];
      }
      attributes.push(['html', 'attr', m[1], attrValue]);
      continue;
    }
